...
export const generateFeatureGuidePDF = (): void => {
  ...
  doc.text('ATS Intel Feature Guide', pageWidth / 2, yPos, { align: 'center' });
  yPos += 15;
  ...
  doc.save('ats-intel-feature-guide.pdf');
};
...
export const generateImplementationChecklistPDF = (): void => {
  ...
  doc.setFontSize(12);
  doc.setFont('helvetica', 'normal');
  doc.text('Step-by-step guide to get started with ATS Intel', pageWidth / 2, yPos, { align: 'center' });
  yPos += 20;
  ...
  doc.save('implementation-checklist.pdf');
};
...
export const generateBestPracticesPDF = (): void => {
  ...
  doc.save('best-practices-guide.pdf');
};
