        ...
        <Card>
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              {getTrendIcon(metrics?.spendTrend.direction || 'stable')}
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">${(metrics?.monthlySpend || 0).toLocaleString()}</div>
            <div className={`text-xs flex items-center gap-1 ${getTrendColor(metrics?.spendTrend.direction || 'stable')}`}>
              {metrics?.spendTrend.percentage || 0}% vs last month
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              Previous: ${(metrics?.previousMonthSpend || 0).toLocaleString()}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              {getTrendIcon(metrics?.applicationsTrend.direction || 'stable')}
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{metrics?.totalApplications || 0}</div>
            <div className={`text-xs flex items-center gap-1 ${getTrendColor(metrics?.applicationsTrend.direction || 'stable')}`}>
              {metrics?.applicationsTrend.percentage || 0}% vs last month
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              Previous: {metrics?.previousMonthApplications || 0}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              {getTrendIcon(metrics?.jobsTrend.direction || 'stable')}
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{metrics?.activeJobs || 0}</div>
            <div className={`text-xs flex items-center gap-1 ${getTrendColor(metrics?.jobsTrend.direction || 'stable')}`}>
              {metrics?.jobsTrend.percentage || 0}% vs last month
            </div>
            <div className="text-xs text-muted-foreground mt-1">
              Previous: {metrics?.previousMonthJobs || 0}
            </div>
          </CardContent>
        </Card>
