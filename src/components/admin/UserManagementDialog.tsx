...
import { Users, Mail, Plus, UserX, Briefcase } from 'lucide-react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { supabase } from '@/integrations/supabase/client';
import { useToast } from '@/hooks/use-toast';
import { useAdminAccess } from '@/hooks/useAdminAccess';
import { getRoleDisplayName } from '@/utils/authHelpers';
import { UserClientAssignmentDialog } from './UserClientAssignmentDialog';
...
type InviteRole = 'user' | 'client' | 'recruiter' | 'moderator' | 'admin';
...
export const UserManagementDialog = ({ organization, trigger }: UserManagementDialogProps) => {
  ...
  const [clientAssignUser, setClientAssignUser] = useState<OrganizationUser | null>(null);
  ...
  const { data: users, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<OrganizationUser[]> => {
      ...
      return profiles.map(profile => {
        ...
        return { ...profile, role: userRole?.role || 'user' } as OrganizationUser;
      });
    },
    enabled: open,
  });
  ...
  const inviteUserMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setInviteEmail('');
      setInviteRole('user');
      toast({ title: 'User Invited', description: 'User has been successfully added to the organization.' });
    },
    onError: (error: any) => {
  ...
  return (
    <>
                    ...
                    <div key={user.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-muted/50 transition-colors">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => setClientAssignUser(user)}
      ...
      <UserClientAssignmentDialog
        user={clientAssignUser ? {
          id: clientAssignUser.id,
          email: clientAssignUser.email,
          organization_id: organization.id,
          organization_name: organization.name,
        } : null}
        isOpen={!!clientAssignUser}
        onClose={() => setClientAssignUser(null)}
      />
    </>
  );
};
