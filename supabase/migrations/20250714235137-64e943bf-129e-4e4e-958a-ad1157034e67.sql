
-- Insert or update the job listing with ID 328
-- First, let's try to update if it exists
UPDATE public.job_listings 
SET description = 'Weekly Home Time • Great Mileage • Stay Active on the Job!

Position Overview
We are now hiring experienced CDL-A drivers for a dedicated fleet based out of St. George, UT. This is an active driver role, ideal for professionals who enjoy hands-on work and regular home time.

Job Details
Average Miles/Week: 1,400

Loads/Week: 1–2

Stops/Load: 2–3

Unloading: Required – use of rollers and climbing in/out of trailers as needed

Delivery Regions
Customer deliveries throughout:

AZ, CA, CO, ID, MT, NE, NV, NM, OR, UT, WA, WY

Schedule
Start Time: 6:00 AM

Work Duration: Up to 14 hours per day

Home Time: Weekly 34-hour reset

Days Off: Vary based on freight; typically weekdays

Special Note for Las Vegas Residents:

2 weeks on, followed by 3 days home

Compensation
Mileage Pay

Pay per Load

Safe & On-Time Mileage Bonus: Up to 3% of Mileage Pay

Equipment
Tandem-Axle Sleeper Trucks

53'' Dry Van Trailers

Automatic Transmissions

All trucks provided by company

Benefits
Weekly Pay

Weekly Home Time

Health Benefits & 401(k)

Paid Time Off (PTO)

Bonus Incentives

Unlimited Cash Referral Program

Full Benefits Package Includes:

Medical, HSA, Dental

Life Insurance & AD&D

PTO & 401(k)

Additional voluntary benefits

For full eligibility details, see the Company''s Benefit Summary and Driver Employee Policy Manual.'
WHERE job_id = '328';

-- If no rows were updated (meaning job_id 328 doesn't exist), let's insert a new record
-- We'll need to get the platform_id and category_id from existing data or use defaults
INSERT INTO public.job_listings (
    job_id,
    title,
    description,
    user_id,
    platform_id,
    category_id,
    city,
    state,
    status
)
SELECT 
    '328',
    'CDL-A Driver - Weekly Home Time - St. George, UT',
    'Weekly Home Time • Great Mileage • Stay Active on the Job!

Position Overview
We are now hiring experienced CDL-A drivers for a dedicated fleet based out of St. George, UT. This is an active driver role, ideal for professionals who enjoy hands-on work and regular home time.

Job Details
Average Miles/Week: 1,400

Loads/Week: 1–2

Stops/Load: 2–3

Unloading: Required – use of rollers and climbing in/out of trailers as needed

Delivery Regions
Customer deliveries throughout:

AZ, CA, CO, ID, MT, NE, NV, NM, OR, UT, WA, WY

Schedule
Start Time: 6:00 AM

Work Duration: Up to 14 hours per day

Home Time: Weekly 34-hour reset

Days Off: Vary based on freight; typically weekdays

Special Note for Las Vegas Residents:

2 weeks on, followed by 3 days home

Compensation
Mileage Pay

Pay per Load

Safe & On-Time Mileage Bonus: Up to 3% of Mileage Pay

Equipment
Tandem-Axle Sleeper Trucks

53'' Dry Van Trailers

Automatic Transmissions

All trucks provided by company

Benefits
Weekly Pay

Weekly Home Time

Health Benefits & 401(k)

Paid Time Off (PTO)

Bonus Incentives

Unlimited Cash Referral Program

Full Benefits Package Includes:

Medical, HSA, Dental

Life Insurance & AD&D

PTO & 401(k)

Additional voluntary benefits

For full eligibility details, see the Company''s Benefit Summary and Driver Employee Policy Manual.',
    auth.uid(),
    (SELECT id FROM platforms LIMIT 1),
    (SELECT id FROM job_categories LIMIT 1),
    'St. George',
    'UT',
    'active'
WHERE NOT EXISTS (SELECT 1 FROM job_listings WHERE job_id = '328');
