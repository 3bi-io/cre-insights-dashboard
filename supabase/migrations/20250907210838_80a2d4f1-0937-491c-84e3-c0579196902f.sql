...
-- 3) Auth helper functions created before we use them in DEFAULTs
...
-- 4) Add organization_id columns (nullable for now) with DEFAULTs where safe
DO $$
BEGIN
  IF NOT EXISTS (
  ...
  -- Meta tables (no default; service functions must set explicitly)
  IF NOT EXISTS (
...
-- 8) Update signup triggers to attach org
