...
-- 3) Add profiles.organization_id BEFORE creating functions that reference it
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='profiles' AND column_name='organization_id'
  ) THEN
    ALTER TABLE public.profiles ADD COLUMN organization_id uuid REFERENCES public.organizations(id) ON DELETE CASCADE;
  END IF;
END$$;

-- 4) Now create helper functions
...
-- 5) Add organization_id columns to other tables (nullable for now) with DEFAULTs where safe
DO $$
BEGIN
  -- user_roles
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='user_roles' AND column_name='organization_id'
  ) THEN
  ...
  -- job_listings
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='job_listings' AND column_name='organization_id'
  ) THEN
    ALTER TABLE public.job_listings ADD COLUMN organization_id uuid REFERENCES public.organizations(id) ON DELETE CASCADE DEFAULT public.get_user_organization_id();
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.campaigns ADD COLUMN organization_id uuid REFERENCES public.organizations(id) ON DELETE CASCADE DEFAULT public.get_user_organization_id();
  END IF;

  -- platforms
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='platforms' AND column_name='organization_id'
  ) THEN
    ALTER TABLE public.platforms ADD COLUMN organization_id uuid REFERENCES public.organizations(id) ON DELETE CASCADE DEFAULT public.get_user_organization_id();
  END IF;

  -- clients
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='clients' AND column_name='organization_id'
  ) THEN
    ALTER TABLE public.clients ADD COLUMN organization_id uuid REFERENCES public.organizations(id) ON DELETE CASCADE DEFAULT public.get_user_organization_id();
  END IF;

  -- chat_sessions
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='chat_sessions' AND column_name='organization_id'
  ) THEN
    ALTER TABLE public.chat_sessions ADD COLUMN organization_id uuid REFERENCES public.organizations(id) ON DELETE CASCADE DEFAULT public.get_user_organization_id();
  END IF;

  -- webhook_configurations
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='webhook_configurations' AND column_name='organization_id'
  ) THEN
    ALTER TABLE public.webhook_configurations ADD COLUMN organization_id uuid REFERENCES public.organizations(id) ON DELETE CASCADE DEFAULT public.get_user_organization_id();
  END IF;

  -- ai_settings
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='ai_settings' AND column_name='organization_id'
  ) THEN
    ALTER TABLE public.ai_settings ADD COLUMN organization_id uuid REFERENCES public.organizations(id) ON DELETE CASCADE DEFAULT public.get_user_organization_id();
  END IF;

  -- meta tables (no default)
  IF NOT EXISTS (
...
-- 6) Backfill data
WITH org AS (SELECT public.get_org_id_by_slug('cr-england') AS id)
UPDATE public.profiles SET organization_id = (SELECT id FROM org) WHERE organization_id IS NULL;
...
-- 9) Update signup triggers to attach org
