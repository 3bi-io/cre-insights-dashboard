...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // --- Real-time Voicemail Detection (shared utility) ---
    const toolCalls = payload.analysis?.tool_calls || [];
    const vmResult = detectVoicemail(toolCalls, formattedTranscript);

    if (vmResult.detected) {
      logger.info('Voicemail detected in webhook', {
        conversationId: payload.conversation_id,
        vmTool: vmResult.toolTriggered,
        vmTranscript: vmResult.transcriptMatch,
      ...
      if (outboundCall) {
        ...
        if ((outboundCall.retry_count || 0) === 0 && !outboundCall.sms_followup_sent && outboundCall.application_id) {
          try {
            await sendVoicemailVerificationSms(supabase, {
              callId: outboundCall.id,
              applicationId: outboundCall.application_id,
              organizationId: outboundCall.organization_id || null,
            });
          } catch (smsErr) {
