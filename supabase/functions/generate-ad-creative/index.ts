...
const logger = createLogger('generate-ad-creative');
...
const BENEFIT_LABELS: Record<string, string> = {
  sign_on_bonus: "$5k Sign-on Bonus", home_weekly: "Home Weekly", new_equipment: "New Equipment",
  full_benefits: "Full Benefits", pet_friendly: "Pet Friendly", no_touch_freight: "No Touch Freight",
  paid_orientation: "Paid Orientation", safety_bonuses: "Safety Bonuses", rider_policy: "Rider Policy",
  direct_deposit: "Direct Deposit", referral_bonus: "Referral Bonus", health_insurance: "Health Insurance",
};

const JOB_TYPE_LABELS: Record<string, string> = {
  long_haul: "Long Haul", regional: "Regional", local: "Local", dedicated: "Dedicated", team: "Team Driving",
};

const ASPECT_RATIO_DIMENSIONS: Record<string, { width: number; height: number }> = {
  "1:1": { width: 1024, height: 1024 }, "16:9": { width: 1024, height: 576 },
  "9:16": { width: 576, height: 1024 }, "4:5": { width: 896, height: 1120 },
};
  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        ...
        hashtags: { type: "array", items: { type: "string" }, description: "3-5 relevant hashtags without the hash symbol" },
...
serve(async (req) => {
  ...
  try {
    ...
    const benefitsList = benefits.map((b) => BENEFIT_LABELS[b] || b).join(", ");
...
Generate engaging ad copy that will attract qualified CDL drivers.`;
    ...
    if (!aiResponse.ok) {
      ...
      logger.error('AI Gateway error', new Error(errorText), { status: aiResponse.status });
...
Style: High-quality stock photo style for ${companyName || "a professional trucking company"}'s social media recruitment campaign.`;
        ...
        logger.info('Generating image', { model: IMAGE_MODEL });
        ...
        if (imageResponse.ok) {
          ...
          if (generatedImageUrl) {
            logger.info('Image generated successfully', { model: IMAGE_MODEL });
            mediaUrl = generatedImageUrl;
          } else {
            logger.info('No image URL in response');
            imageError = "Image generation returned no image. Try again.";
          }
        } else {
          const errText = await imageResponse.text();
          logger.error('Image generation failed', new Error(errText), { status: imageResponse.status });
          
          if (imageResponse.status === 429) imageError = "Image generation rate limited. Try again in a moment.";
          else if (imageResponse.status === 402) imageError = "AI credits exhausted for image generation. Please add credits.";
          else imageError = "Image generation failed. Text content was generated successfully.";
        }
      } catch (imgErr: unknown) {
        logger.error('Image generation error', imgErr);
        imageError = "Image generation encountered an error. Text content was generated successfully.";
      }
    }

    return new Response(
      JSON.stringify({ success: true, content: generatedContent, mediaUrl, imageError }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error: unknown) {
    logger.error('Generate ad creative error', error);
    return new Response(
      JSON.stringify({ success: false, error: error instanceof Error ? error.message : "Failed to generate ad creative" }),
