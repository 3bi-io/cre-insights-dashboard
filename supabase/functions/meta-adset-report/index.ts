...
serve(async (req) => {
  ...
  try {
    ...
    logger.info('Report requested', { userId: user.id });
    ...
    logger.info('Found leads in date range', { count: leadsInRange?.length || 0 });
    ...
    logger.info('Unique ad sets with leads', { count: adSetIdsWithLeads.length });
    ...
    if (adSetsError) {
      logger.error('Error fetching ad sets', adSetsError);
      throw adSetsError;
    }

    logger.info('Found ad sets with leads', { count: adSets?.length || 0 });
    ...
    logger.info('Report generated successfully', { 
      totalAdSets: summary.totalAdSets, 
